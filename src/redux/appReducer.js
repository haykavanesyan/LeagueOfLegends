    ...
    case "ADD_RESULT":
      return {
        ...
        result: [
          {
            id: action.id,
            image: "https://cdn-images.win.gg/" + action.image,
            title: action.title,
...
export let deleteResult = () => {
  return { type: "DELETE_RESULT" };
};
export let addResult = (id, image, title, description) => {
  return { type: "ADD_RESULT", id, image, title, description };
};
export let addChosen = (item) => {
  return { type: "ADD_CHOSEN", item };
};
export let removed = (item, id) => {
...
export let searchThunk = (value) => {
  return (dispatch) => {
    ...
    SearchAPI(value).then((res) => {
      if (!!res.data[0]) {
        res.data[0].documents.map((e) => {
          dispatch(
            addResult(e.id, e.images.square.filePath, e.title, e.description)
          );
        });
      }
    });
